...
def convert_avi_to_wav(input_folder, output_folder, file_name):
    input_file_path = os.path.join(input_folder, file_name)
    output_file_name = os.path.splitext(file_name)[0] + ".wav"
    output_file_path = os.path.join(output_folder, output_file_name)
    print("Converting AVI to wav format")
...
if __name__ == "__main__":
    input_folder = input("Enter the path of the folder containing the AVI file: ")
    output_folder = input("Enter the path of the folder to save the WAV file: ")
    file_name = input("Enter the name of the AVI file (including the extension): ")
    convert_avi_to_wav(input_folder, output_folder, file_name)
